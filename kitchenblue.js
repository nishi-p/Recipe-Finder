...
//Always test for different inputs to test errors.

//Cross-site scripting --> Hackers can sneak malicious code into the server by
//entering code snippets in web forms.

/* JSON cannot encapsulate text, numeric data and embedded images as JSON is a
text format and cannot communicate binary data like that. One way to get around
that is to use a URL that points to the object, like Recipe Puppy did with recipe
 thumbnail URLs.
*/
...
//url before api --> cross-origin problems
  request = $.ajax({
...
// and removes them if found --> Cross-site scripting attacks
